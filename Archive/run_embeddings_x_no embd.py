...
ontology_path = os.path.join(kg_base_dir, 'heart/heart_small.owl')
...
# -----------------
# Print sample entities and check for missing patient keys
# print("Sample entities in entity_to_id:")
# for i, (entity, entity_id) in enumerate(triples_factory.entity_to_id.items()):
#     print(f"{entity}: {entity_id}")
#     if i >= 10:
#         break

patient_keys = [f"http://www.semanticweb.org/heart_ontology#Patient_{i}" for i in range(243)]
# missing_patient_keys = [key for key in patient_keys if key not in triples_factory.entity_to_id]
# print(f"Missing patient keys in entity_to_id: {len(missing_patient_keys)}")
# print(f"Example missing patient keys: {missing_patient_keys[:10]}")
    ...
    for model_name, model, param in models_with_params:
        for X_name, X in Xs.items():
            ...
            for train_index, test_index in skf.split(X, y):
                ...
                train_uris = map_indices_to_uris(y_train.index, "http://www.semanticweb.org/heart_ontology")
                test_uris = map_indices_to_uris(y_test.index, "http://www.semanticweb.org/heart_ontology")
                ...
                # Generate embeddings
                embeddings_transh, entity_to_id_transh = pykeen_embeddings(
                    TransH, dimension=128, triples_factory_train=train_triples_factory,
                    triples_factory_test=test_triples_factory)
                embeddings_distmult, entity_to_id_distmult = pykeen_embeddings(
                    DistMult, dimension=128, triples_factory_train=train_triples_factory,
                    triples_factory_test=test_triples_factory)

                embedding_methods = {
                    'TransH': (embeddings_transh, entity_to_id_transh),
                    'DistMult': (embeddings_distmult, entity_to_id_distmult),
                }

                for method_name, (embeddings, entity_to_id) in embedding_methods.items():
                    # Filter the embeddings to match the samples in X_train and X_test
                    train_indices = [entity_to_id[uri] for uri in train_uris if uri in entity_to_id]
                    test_indices = [entity_to_id[uri] for uri in test_uris if uri in entity_to_id]
                    ...
                    #
                    # print(f" valid_test_indices  ----> {valid_test_indices}")

                    # print(f"valid_train_indices: {valid_train_indices}")
                    ...
                    X_test_combined = X_test
                        # np.concatenate((X_test_filtered, test_embeddings), axis=1)

                    df_train = pd.DataFrame(X_train_filtered, columns=df.columns[:-1])
                    df_test = X_test
                        # pd.DataFrame(X_test_filtered, columns=df.columns[:-1])


    #
                    # Add the 'patient_key' column to df_train and df_test
                    df_train['patient_key'] = map_indices_to_uris(valid_train_indices, "http://www.semanticweb.org/heart_ontology")
                    # df_test['patient_key'] = map_indices_to_uris(df_test.index, "http://www.semanticweb.org/heart_ontology")


                    df_train = add_distance_features(df_train, embeddings, entity_to_id, method_name, y_train_filtered)
                    # print(f"df_train after add_distance_fe: {df_train}")
                    ...
                    #TODO:  Check if you can add embeddings into the testing set.

                    scenarios = {
                        'Baseline': (X_train, X_test),
                        f'{method_name} Only': (train_embeddings, X_test),
                        f'{method_name} + Tabular': (X_train_combined, X_test),
                        f'{method_name} FE Only': (X_train_fe, X_test),
                        f'{method_name} + Tabular + FE': (X_train_combined_fe, X_test),
                    }
                    ...
                    for scenario_name, (X_train_final, X_test_final) in scenarios.items():
                        ...
                        # print(f"X_test_final: ----- {X_test_final}")

                        y_pred = model_instance.predict(X_test_final)
                        if model_name == 'NN':
                            y_pred = (y_pred > 0.5).astype(int).flatten()
                        # print(f"y_test_filtered: {y_test}")
